...
async function handleGet(request: Request, env: Env): Promise<Response> {
    ...
    if (keys.length > 0) {
        ...
        for (const [table, tableKeys] of Object.entries(groupedKeys)) {
            ...
            const statement = env.DB.prepare(`SELECT key, value FROM ${table} WHERE key IN (${placeholders})`).bind(
                ...tableKeys,
            );
        ...
        const playbackResult = await env.DB.prepare("SELECT key, value FROM playback_store").all();
        const favoriteResult = await env.DB.prepare("SELECT key, value FROM favorites_store").all();
        rows = [
...
async function handleDelete(request: Request, env: Env): Promise<Response> {
    ...
    keys.forEach((key) => {
        const table = getTableForKey(key);
        groupedStatements[table].push(env.DB.prepare(`DELETE FROM ${table} WHERE key = ?1`).bind(key));
    });
