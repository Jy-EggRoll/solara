...
function analyzeImageColors(image) {
    ...
    const dominantHsl = rgbToHsl(binR[bestBin] / maxWeight, binG[bestBin] / maxWeight, binB[bestBin] / maxWeight);
...
function buildGradientStops(accent, isMonochrome = false) {
    ...
    if (isRedFamily) {
        ...
        lc = [
            hslToHex({ h: warmH, s: redS, l: 0.7 }),
            hslToHex({ h: (warmH + 10) % 360, s: Math.max(0.12, redS - 0.03), l: 0.73 }),
            hslToHex({ h: warmH, s: Math.max(0.1, redS - 0.05), l: 0.76 }),
        ];
    } else {
        // 常规彩色：浅色模式明度控制在 0.72~0.80 舒适区间，杜绝惨白死光
        const lightS = clamp(accent.s * 0.26 + 0.06, 0.14, 0.3);
        lc = [
            hslToHex({ h: accent.h, s: lightS, l: clamp(accent.l * 0.1 + 0.72, 0.72, 0.79) }),
            hslToHex({
                h: (accent.h + 16) % 360,
                s: Math.max(0.11, lightS - 0.03),
                l: clamp(accent.l * 0.08 + 0.75, 0.74, 0.81),
            }),
    ...
    const dc = [
        ...
        hslToHex({
            ...
            l: clamp(accent.l * 0.06 + 0.12, 0.1, 0.18),
        }),
    ...
    return {
        light: {
            primaryColor: hslToHex({
                h: accent.h,
                s: clamp(accent.s * 0.4 + 0.08, 0.18, 0.45),
                l: clamp(accent.l * 0.16 + 0.34, 0.3, 0.42),
            }),
            primaryColorDark: hslToHex({
                h: accent.h,
                s: clamp(accent.s * 0.45 + 0.05, 0.2, 0.5),
                l: clamp(accent.l * 0.12 + 0.24, 0.22, 0.34),
            }),
        },
        dark: {
            primaryColor: hslToHex({
                h: accent.h,
                s: clamp(accent.s * 0.36 + 0.06, 0.18, 0.38),
                l: clamp(accent.l * 0.12 + 0.36, 0.32, 0.44),
            }),
            primaryColorDark: hslToHex({
                h: accent.h,
                s: clamp(accent.s * 0.4 + 0.04, 0.2, 0.42),
                l: clamp(accent.l * 0.08 + 0.26, 0.22, 0.32),
            }),
        },
    };
}
    ...
    router.get("/", async (req, res) => {
        ...
        if (!upstream.ok) {
            return res
                ...
                .json({ error: `Upstream request failed with status ${upstream.status}` });
        }
